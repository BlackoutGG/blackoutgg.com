...
export default {
  ...
  methods: {
    async save() {
      ...
      //   const results = await this.$axios.post("/api/admin/users/create", data);
      //   this.reset();
      //   this.$store.dispatch(types.actions.TOGGLE_BAR, {
      //     text: `Created user: ${results.username}`
      //   });
      // } catch (err) {
      //   console.log(err.response.data);
      //   const error = err.response.data[0].msg
      //     ? err.response.data[0].msg
      //     : err.response.data
      //     ? err.response.data
      //     : err;
      //   this.$store.dispatch(types.actions.TOGGLE_BAR, { text: err });
      // } finally {
      //   this.isSending = false;
      // }
    },
